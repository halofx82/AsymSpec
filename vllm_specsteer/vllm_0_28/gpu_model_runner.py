...
class GPUModelRunner(
        ...
        if self.speculative_config and get_pp_group().is_last_rank:
            ...
            if self.speculative_config.method == "custom_class":
                ...
                if self.speculative_config.method == "specsteer":
                    # SpecSteer (AsymSpec) — extends DraftModelProposer with
                    # asymmetric context, δ-fusion, and CDA gate. Imported
                    # lazily to keep specsteer_model.py optional.
                    from vllm.v1.spec_decode.specsteer_model import (
                        SpecSteerProposer,
                    )
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        if (self.speculative_config is not None
            ...
            reset_ids = (set(scheduler_output.finished_req_ids)
                         ...
                         | set(scheduler_output.scheduled_cached_reqs.resumed_req_ids))
            self.drafter.invalidate_requests(reset_ids)
    ...
    def _specsteer_sample(
        ...
        from vanilla greedy spec decoding.
        """
        import os
        from vllm.v1.outputs import SamplerOutput
        from vllm.v1.sample.specsteer_sampler import specsteer_greedy_sample

        # Target (LLM) logits at drafted positions and bonus positions.
        ...
        # SPECSTEER_BONUS_MODE selects the K+1 bonus token source.
        # llm     — bonus = argmax(target_logits) at the bonus position
        #           (default; matches Leviathan spec decode semantics)
        # none    — emit no bonus (placeholder -1; sampler skips it)
        # drafter — bonus from drafter's K+1-th forward (NOT IMPLEMENTED in
        #           this snapshot: the sampler path doesn't expose drafter
        #           K+1 bonus logits; commit that wiring first)
        bonus_mode = os.environ.get("SPECSTEER_BONUS_MODE", "llm").lower()
        if bonus_mode == "llm":
            bonus_token_ids = bonus_logits.argmax(
                dim=-1, keepdim=True,
            ).to(torch.int32)
        elif bonus_mode == "none":
            bonus_token_ids = torch.full(
                (bonus_logits.shape[0], 1), -1,
                dtype=torch.int32, device=bonus_logits.device,
            )
        elif bonus_mode == "drafter":
            raise NotImplementedError(
                "SPECSTEER_BONUS_MODE=drafter is not wired in this "
                "snapshot: the sampler path does not expose drafter "
                "K+1 bonus logits. Implement that wiring before using "
                "drafter-bonus runs as reproducible evidence."
            )
        else:
            raise ValueError(
                f"SPECSTEER_BONUS_MODE={bonus_mode!r} not in "
                "{'llm', 'none', 'drafter'}"
            )
        ...
        assert draft_logits_list, "SpecSteerProposer produced no drafter logits"
        # [K, batch, V] → [batch, K, V] → [batch*K, V]
        stacked = torch.stack(draft_logits_list, dim=1).contiguous()
        batch_size, K, V = stacked.shape
        num_tokens = target_logits.shape[0]
        if batch_size * K != num_tokens:
        ...
        if base_list and len(base_list) == K:
            ...
            if base_stacked.shape[:2] == (batch_size, K):
                base_logits = base_stacked.view(num_tokens, V).to(torch.float32)
            else:
                raise RuntimeError(
                    "SpecSteer base_logits shape mismatch in sampler: "
                    f"got {tuple(base_stacked.shape[:2])}, expected "
                    f"{(batch_size, K)}; refusing silent fallback to "
                    "base = aug."
                )
        else:
            raise RuntimeError(
                "SpecSteer base_logits missing in sampler "
                "(_base_logits_per_pos is None or length != K); refusing "
                "silent fallback to base = aug. Set "
                "SPECSTEER_ALLOW_FALLBACK=1 on the proposer side AND wire "
