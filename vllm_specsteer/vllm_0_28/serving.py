...
class _AsymSpecChatInputs:
    """The two independently rendered prompt views for one chat request."""
...
class OpenAIServingChat(GenerateBaseServing):
    def __init__(
        ...
        *,
        ...
        # The frontend reads the authoritative engine configuration; no
        # duplicate CLI or environment configuration is maintained here.
        ...
        self._asymspec_enabled = (
            ...
            and getattr(self._speculative_config, "method", None) == "specsteer"
        )
    ...
    async def _render_asymspec_chat_request(
        ...
        """Render full and recent-context prompts through ``OnlineRenderer``.

        The engine receives only the compressed prompt.  The full prompt is
        carried in the server-owned sampling extra argument for the SpecSteer
        full-context drafter.
        """
        ...
        # Preserve OpenAIServingChat's normal model/engine checks while using
        # the renderer directly for both views.
        error_check_ret = await self._check_model(request)
        if error_check_ret is not None:
            return error_check_ret
        if self.engine_client.errored:
        ...
        num_speculative_tokens = int(
            getattr(self._speculative_config, "num_speculative_tokens", 0) or 0
        )
        full_limit = self.model_config.max_model_len
        if len(full_prompt_token_ids) + max_tokens + num_speculative_tokens > full_limit:
            return self.online_renderer.create_error_response(
        ...
        main_limit = (
            getattr(self._speculative_config, "specsteer_main_max_model_len", None)
            or full_limit
        )
        main_prompt_budget = main_limit - max_tokens - num_speculative_tokens
        if main_prompt_budget <= 0:
            return self.online_renderer.create_error_response(
                "specsteer_main_max_model_len leaves no room for the requested "
                "completion and speculative-token headroom"
            )
        ...
        main_engine_input: EngineInput | None = None
        ...
        for start in range(len(turns) - 1, -1, -1):
            ...
            main_result = await self.online_renderer.render_chat(compressed_request)
            if isinstance(main_result, ErrorResponse):
                return main_result
            _, main_engine_inputs = main_result
            if len(main_engine_inputs) != 1:
                return self.online_renderer.create_error_response(
                    "AsymSpec serving requires exactly one rendered compressed prompt"
                )
            candidate_input = main_engine_inputs[0]
            candidate_tokens = len(
                self._extract_prompt_components(candidate_input).token_ids or []
            )
            if candidate_tokens <= main_prompt_budget:
                main_engine_input = candidate_input
                main_prompt_tokens = candidate_tokens
                continue
            break

        if main_engine_input is None:
            return self.online_renderer.create_error_response(
                "Pinned instructions plus the newest user interaction exceed "
                "the AsymSpec compressed-context capacity"
            )
        logger.info(
            "AsymSpec request: full=%d main=%d max_new=%d K=%d strategy=recent",
            len(full_prompt_token_ids),
            main_prompt_tokens,
            max_tokens,
        ...
        return _AsymSpecChatInputs(
            ...
            engine_input=main_engine_input,
    ...
    async def _create_chat_completion(
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            if request.use_beam_search:
                ...
                if asymspec_inputs is not None:
                    # Avoid mutating request.vllm_xargs, which is reused by
                    # request logging and must never override server policy.
