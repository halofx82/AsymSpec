...
def get_kv_cache_groups(vllm_config, kv_cache_spec):
    groups = _get_kv_cache_groups_without_specsteer(vllm_config, kv_cache_spec)
    if not (vllm_config.speculative_config is not None
            and vllm_config.speculative_config.method == "specsteer"):
        return groups
    result = []
    for group in groups:
        partitions = [
            [n for n in group.layer_names if "draft_model." not in n],
            [n for n in group.layer_names if "draft_model." in n],
        ]
        for names in partitions:
            if not names:
                continue
            specs = {n: kv_cache_spec[n] for n in names}
            if is_kv_cache_spec_uniform(specs):
                result.extend(_get_kv_cache_groups_uniform_spec(specs))
            else:
...
def _get_kv_cache_groups_without_specsteer(
