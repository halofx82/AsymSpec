...
class SpecSteerKVCacheCoordinator(KVCacheCoordinatorNoPrefixCache):
    """Independent full-context token accounting on the scheduler process."""
    ...
    def _view_tokens(self, gid, request_id, num_tokens):
        ...
        return max(0, num_tokens + offset)

    def free(self, request_id):
        self.aug_offsets.pop(request_id, None)
    ...
    def get_num_blocks_to_allocate(
        ...
        for i, manager in enumerate(self.single_type_managers):
            ...
            if isinstance(manager, CrossAttentionManager):
                ...
                num_blocks_to_allocate += manager.get_num_blocks_to_allocate(
                    request_id,
                    self._view_tokens(i, request_id, num_tokens),
                    new_computed_blocks[i],
                    total_computed_tokens,
                    num_local_computed_tokens,
                    view_main_tokens,
